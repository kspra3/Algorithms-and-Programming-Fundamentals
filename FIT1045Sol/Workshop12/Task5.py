...
def startQuiz():
    ...
    randomSet = random.sample(range(0,len(questions)),len(questions))
    
    for i in range (len(randomSet)):
        print ('-' * 40)
        randIndex = randomSet[i]
        
        for j in range (len(questions[randIndex])):
            print(questions[randIndex][j])
            
        input_answer = input('Answer is: ')
        if input_answer == answers[randIndex]:
            correct += 1
            print('Correct')
        else:
            print('Correct answer is',answers[randIndex])
